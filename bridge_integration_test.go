		...
		for _, tc := range cases {
			for _, streaming := range []bool{true, false} {
				t.Run(fmt.Sprintf("%s/streaming=%v", tc.name, streaming), func(t *testing.T) {
					...
					logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug)
					b, err := aibridge.NewRequestBridge(
						ctx, []aibridge.Provider{provider.NewAnthropic(anthropicCfg(upstream.URL, apiKey), bCfg)},
						recorderClient, mcp.NewServerProxyManager(nil, testTracer), logger, nil, testTracer)
					require.NoError(t, err)
					...
					req := createAnthropicMessagesReq(t, mockBridgeSrv.URL, reqBody)
					req.Header.Set("Anthropic-Beta", "interleaved-thinking-2025-05-14,effort-2025-11-24,context-management-2025-06-27,prompt-caching-scope-2026-01-05")
					resp, err := http.DefaultClient.Do(req)
					require.NoError(t, err)
					defer resp.Body.Close()
