...
func TestCircuitBreaker_HalfOpenAndRecovery(t *testing.T) {
	...
	// Mock upstream that can switch between error and success.
	// x-should-retry: false is required to disable SDK automatic retries.
	mockUpstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		upstreamCalls.Add(1)
		w.Header().Set("Content-Type", "application/json")
		w.Header().Set("x-should-retry", "false")
		if returnError.Load() {
			...
			_, _ = w.Write([]byte(`{"id":"msg_123","type":"message","role":"assistant","content":[{"type":"text","text":"hi"}],"model":"claude-sonnet-4-20250514","stop_reason":"end_turn","usage":{"input_tokens":10,"output_tokens":5}}`))
		}
	}))
	...
	provider := aibridge.NewAnthropicProvider(aibridge.AnthropicConfig{
		...
		CircuitBreaker: &aibridge.CircuitBreakerConfig{
			FailureThreshold: 2,
			Interval:         time.Minute,
			Timeout:          50 * time.Millisecond, // Short timeout for faster test
			MaxRequests:      1,
		},
	}, nil)
	...
	// Switch upstream to success before recovery
	returnError.Store(false)

	// Wait for timeout, then make a request to trigger recovery
	time.Sleep(60 * time.Millisecond)

	// This request triggers recovery: half-open -> probe succeeds -> closed
	resp := makeRequest()
	assert.Equal(t, http.StatusOK, resp.StatusCode)

	// Verify circuit is now closed (state = 0)
	state = promtest.ToFloat64(metrics.CircuitBreakerState.WithLabelValues(aibridge.ProviderAnthropic, "/v1/messages"))
	assert.Equal(t, 0.0, state, "CircuitBreakerState should be 0 (closed)")

	// Additional requests should succeed
	resp = makeRequest()
	assert.Equal(t, http.StatusOK, resp.StatusCode)
}
