...
func TestAWSBedrockIntegration(t *testing.T) {
	...
	t.Run("SigV4 signed headers", func(t *testing.T) {
		...
		// the request as-is without SigV4 signing, so proxy headers are
		// safe to include. The passthrough router sets its own
		// X-Forwarded-* headers in its Director. This verifies they
		// arrive upstream.
		t.Run("passthrough proxy sets own forwarded headers", func(t *testing.T) {
			...
			assert.NotEmpty(t, last.Header.Get("X-Forwarded-For"),
				"passthrough should set X-Forwarded-For")
			assert.NotEmpty(t, last.Header.Get("X-Forwarded-Host"),
				"passthrough should set X-Forwarded-Host")
			assert.NotEmpty(t, last.Header.Get("X-Forwarded-Proto"),
				"passthrough should set X-Forwarded-Proto")
		})
	})
}
