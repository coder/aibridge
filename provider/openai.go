...
func (p *OpenAI) CreateInterceptor(w http.ResponseWriter, r *http.Request, tracer trace.Tracer) (_ intercept.Interceptor, outErr error) {
	...
	case routeResponses:
		...
		if req.Stream {
			interceptor = responses.NewStreamingInterceptor(id, &req, payload, p.baseURL, p.key)
		} else {
			interceptor = responses.NewBlockingInterceptor(id, &req, payload, p.baseURL, p.key)
		}
