...
func (p *Copilot) CreateInterceptor(_ http.ResponseWriter, r *http.Request, tracer trace.Tracer) (_ intercept.Interceptor, outErr error) {
	...
	case routeCopilotChatCompletions:
		...
		if req.Stream {
			interceptor = chatcompletions.NewStreamingInterceptor(id, &req, cfg, r.Header, tracer)
		} else {
			interceptor = chatcompletions.NewBlockingInterceptor(id, &req, cfg, r.Header, tracer)
		}
	...
	case routeCopilotResponses:
		...
		if req.Stream {
			interceptor = responses.NewStreamingInterceptor(id, &req, payload, cfg, req.Model, r.Header, tracer)
		} else {
			interceptor = responses.NewBlockingInterceptor(id, &req, payload, cfg, req.Model, r.Header, tracer)
		}
